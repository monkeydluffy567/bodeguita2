...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 02-03-2020 a las 06:09:26
...
where productos.nombre=prm_criterio or marcas.marca=prm_criterio or subtipos.nombre=prm_criterio ;
...
set var_id_marca =(select id_marca from marcas where marca=prm_marca);
set var_id_unidad =(select id_unidad from unidades where unidad=prm_unidad);
set var_id_subtipo =(select id_subtipo from subtipos where nombre=prm_subtipo);
...
select id_usuario,email,apepat,apemat,nombre,telefono,dni,dirreccion,tipo,estado from usuarios
